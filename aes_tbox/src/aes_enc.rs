...
#[derive(Block, PartialEq, Eq, Debug)]
pub struct State {
    bytes: [u8; 16],
}
...
impl TBoxes {
    ...
    fn tbox_lup(&self, state: &mut State) {
        let e0: u32 = self.tbox0[state[0] as usize]
            ^ self.tbox1[state[5] as usize]
            ^ self.tbox2[state[10] as usize]
            ^ self.tbox3[state[15] as usize];
        let e1: u32 = self.tbox0[state[4] as usize]
            ^ self.tbox1[state[9] as usize]
            ^ self.tbox2[state[14] as usize]
            ^ self.tbox3[state[3] as usize];
        let e2: u32 = self.tbox0[state[8] as usize]
            ^ self.tbox1[state[13] as usize]
            ^ self.tbox2[state[2] as usize]
            ^ self.tbox3[state[7] as usize];
        let e3: u32 = self.tbox0[state[12] as usize]
            ^ self.tbox1[state[1] as usize]
            ^ self.tbox2[state[6] as usize]
            ^ self.tbox3[state[11] as usize];
